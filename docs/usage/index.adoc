...
applications to seamlessly authenticate with their Microsoft® Windows credentials.
...
When using bundlers such as Rollup, Webpack, etc, the `"IWA.WebApi"` object must be
...
with a logon UI. The page either starts the Integrated Windows Authentication process
automatically or allows the user to initiate it by clicking a UI element such as buttons.

In your IWA sign-in controller class, create an instance of an `IWA.WebApi` class
